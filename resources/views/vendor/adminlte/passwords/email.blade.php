...
@section('body')
    <div class="login-box">
        ...
        <div class="login-box-body">
            ...
            <p class="login-box-msg">{{ trans('adminlte::adminlte.membermail') }}</p>
            @if (session('status'))
                <div class="alert alert-success">
                    {{ session('status') }}
                </div>
            @endif
            <form class="form-group" action="{{ url(config('adminlte.password_email_url', 'password/email')) }}" method="post">
                {!! csrf_field() !!}
